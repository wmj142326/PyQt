...
# str = "python scripts/demo_inference.py --cfg configs/coco/resnet/256x192_res50_lr1e-3_1x.yaml --checkpoint input/exp/2_AlphaPose/final_DPG.pth --outdir out/webcam_1 --vis --webcam 0"
...
class codeShow(QMainWindow, Ui_MainWindow):
    ...
    def PrepParameters(self):
        ...
        self.CodePath = 'python social-distancing-prediction-master/code/inference/main_simple.py'  # python py文件
        self.lineEdit_code1.setText(self.CodePath)
        self.CodePath = ' social-distancing-prediction-master/test/VID379.mp4'  # 视频文件
        self.lineEdit_code2.setText(self.CodePath)
        self.CodePath = ''  # 暂时无法选择模型
        self.lineEdit_code3.setText(self.CodePath)
        self.CodePath = ' social-distancing-prediction-master/test/output_379_simple'  # 日志文件保存路径
        self.lineEdit_code4.setText(self.CodePath)
        self.CodePath = ' --pred_vis_path social-distancing-prediction-master/test/visualization  --is_baseline'  # 可视化视频帧保存路径
        self.lineEdit_code5.setText(self.CodePath)
    ...
    def connect(self):
        ...
        # self.pushButton_exit.clicked.connect(self.ExitCamera)
        ...
        self.pushButton_OK.clicked.connect(self.runCamera)
    ...
    def chooseNet(self):
        if self.comboBox_NetChoose.currentIndex() == 0:
            ...
            # self.pushButton_code5.setText('变啦')
        elif self.comboBox_NetChoose.currentIndex() == 1:
            self.pushButton_code1.setText('model')
            ...
            self.pushButton_code5.setText('visulization')
...
# 输入运行
    def runCamera(self):
        self.Text_Msg.clear()
        subprocess.run(r"python ex1.py")
        self.Text_Msg.setText('结束')
...
# 输出显示
    ...
    def imshow(self):
        ...
        if self.radioButton_video.isChecked() == False:
            ...
            for imgpath in imglist:
                ...
                # jpg = QtGui.QPixmap(imgpath).scaled(self.Box_out1.width(), self.Box_out1.height())
                # self.Box_out1.setPixmap(jpg)
        ...
        #     ret, img2 = self.camera.read()
        #     # img2 = cv2.resize(img, (450, 300))
        #     img2 = qimage2ndarray.array2qimage(img2)
        #     self.Box_out3.setPixmap(QtGui.QPixmap(img2))
        #     self.Box_out3.show()
        #
        #     cv2.waitKey(10)
        # self.camera.release()
    ...
    def button_open_camera(self):
        if self.timer_camera.isActive() == False:  # 若定时器未启动
            ...
            if flag == False:  # flag表示open()成不成功
                ...
                self.Text_Msg.setText('请检查相机于电脑是否连接正确')
            else:
    ...
    #
    # def StopCamera(self):
    #     if self.pushButton_stop.text() == '暂停':
    #         self.pushButton_stop.setText('继续')
    #         self.Timer.stop()
    #     elif self.pushButton_stop.text() == '继续':
    #         self.pushButton_stop.setText('暂停')
    #         self.Timer.start(1)
    #
    # def ExitCamera(self):
    #     self.Timer.stop()
    #     self.camera.release()
    #     self.Text_Msg.setPlainText('Exiting the application..')
    #     QCoreApplication.quit()
    # def send_msg(self):
    #     try:
    #         if self.text_edit.toPlainText() != '1':
    #             os.popen("python time_camera.py")
    #         # os.popen(self.text_edit.toPlainText())
    #     except:
    #         print("出现错误")
    #         # subprocess.Popen(r"cd C:\Users\86150\Desktop && python terminal.py")
    #         # subprocess.Popen(self.text_edit.toPlainText())
    #
    # def show_msg(self):
    #     print('Begin')
    #     loop = QEventLoop()
